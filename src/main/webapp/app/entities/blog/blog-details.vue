    ...
    <div class="col-8">
      <div v-if="blog">
        <h2 class="jh-entity-heading" data-cy="blogDetailsHeading">
          <span v-text="$t('cmsApp.blog.detail.title')">Blog</span> {{ blog.id }}
        </h2>
        <dl class="row jh-entity-details">
          <dt>
            <span v-text="$t('cmsApp.blog.name')">Name</span>
          </dt>
          <dd>
            <span>{{ blog.name }}</span>
          </dd>
          <dt>
            <span v-text="$t('cmsApp.blog.handle')">Handle</span>
          </dt>
          <dd>
            <span>{{ blog.handle }}</span>
          </dd>
          <dt>
            <span v-text="$t('cmsApp.blog.user')">User</span>
          </dt>
          <dd>
